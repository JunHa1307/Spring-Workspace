...
public class BoardServiceImpl implements BoardService{
	...
	public void selectBoardSearch(Map<String, Object> paramMap, Map<String, Object> map) {
		int listCount = boardDao.selectBoardSearchCount(paramMap);
		int pageLimit = 10;
		int boardLimit = 5;
		PageInfo pi = pagination.getPageInfo(listCount, (int)paramMap.get("currentPage"), pageLimit, boardLimit);
		ArrayList<Board> list = boardDao.selectBoardSearch(pi, paramMap);
		map.put("pi", pi);
		map.put("list", list);
	}
	 ...
	 * 
	 * 예외가 발생하면 자동으로 rollback 처리를 수행함
	 * service , service 내부의 코드 수핸시점에서 에러 발생 시 자동 rollback이 된다.
	 * 
	 * 방법 1) aop 활용하는 방법
	 * 
	 * 방법 2) @Transactional 어노테이션 활용하여 선언적 트랜잭션 처리
	 * 		 -> RuntimeException에 대한 처리를 기본값으로 가진다.
	 * 
	 * rollbackFor : rollback을 수행하기 위한 예외의 종류를 작성하는 부분
	 */
	@Transactional(rollbackFor = {Exception.class}) // 모든종류의 예외가 발생하면 rollback시키겠다라고 선언
	public int insertBoard(Board b, List<MultipartFile> list, String webPath, String serverFolderPath) throws Exception {
		...
		if(boardNo > 0 && list != null) {
			...
			// boardImageList : db에 등록한 데이터를 모아놓은 컬랙션
...
//											비어있지 않다? 등록한 이미지가 있음
			...
			if(!boardImageList.isEmpty()) {
				...
				if(result == boardImageList.size()) {
					...
					throw new Exception("이미지 등록 시 예외발생");
				}
			}
		}
			...
			if(list != null) {
				...
				if(result > 0) {
					...
					for(BoardImg img : boardImgList) {
						...
						// 결과값 1 -> 수정작업이 잘 이루어졌다 -> 기존 이미지가 있었다.
						// 결과값 0 -> 수정작업이 실패함 -> 이미지가 없었다.
						
						// 6) 결과값이 0 -> update에는 실패했고, 실제로 db에는 올라가야할 데이터이기 때문에 insert시켜준다
						if(result == 0) {
				...
				// 7) 업로드된 이미지가 있다면, 서버에 저장하기(이 부분은 Utils로 대체함)
			}
		}
		return result;
	}
